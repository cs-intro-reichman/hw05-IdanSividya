...
public class GameOfLife {
	...
	private static void test2(String fileName) {
		int[][] board = read(fileName);
		int sum = count (board, 3, 3);
		System.out.println(sum);
		int cell = cellValue (board, 2, 3);
	...
	public static int count(int[][] board, int i, int j) {
		int count = board[i-1][j-1] + board[i-1][j] + board[i-1][j+1] + board[i][j-1] + board[i][j+1] + board[i+1][j-1] + board[i+1][j] + board[i+1][j+1];
		return count;
	}
